                    ...
                    <form action="{{ route('dashboard.article.update' , $article->id) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="row mbn-20">
                            ...
                            <div class="col-12 mb-20">
                                ...
                                <input type="text" name="tags[]" value="{{old( 'title' , $tags)}}" data-role="tagsinput"
                                       class="form-control">
                            </div>
                            @error('tags')
                            {{$message}}
                            ...
                            <div class="col-12 mb-20" id="divWrapper">
                                <select name="type_file" id="fileTypeSelect" onchange="checkSelectValue()">
                                    <option value="blog" {{$article->type_file === "blog" ? 'selected' : ""}}>مقاله</option>
                                    <option value="video" {{$article->type_file === "video" ? 'selected' : ""}}>ویدیو</option>
                                    <option value="podcast" {{$article->type_file === "podcast" ? 'selected' : ""}}>پادکست
                                    </option>
                                </select>
                            </div>
                            @error('type_file')
                            ...
                            <div class="col-12 mb-20" style="display: none" id="pathFileInput">
                                ...
                                <input type="text" name="path_file" id="path_file" aria-describedby="button-path-file"
                                       value="{{old('path_file' , $article->path_file)}}"
                                       class="form-control">
                            ...
                            <div class="col-12 mb-20">
                                <div class="form-group">
                                    <label class="inline"><input name="status"
                                                                 value="0" {{$article->status == 0 ? 'checked' : ""}}
                                                                 type="radio">پیش نویس</label>

                                    <label class="inline"><input name="status"
                                                                 value="1" {{$article->status == 1 ? 'checked' : ""}}
                                                                 type="radio">منتشر شده</label>
                                </div>
                            </div>
                            @error('status')
                            {{$message}}
